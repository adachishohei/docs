...
- Delete the existing API key by setting `CreateAPIKey` to `1` in the `amplify/backend/api/<apiName>/parameters.json` file and execute `amplify push`.
- Create a new API key by setting `CreateAPIKey` to `0` in the `amplify/backend/api/<apiName>/parameters.json` file and execute `amplify push`.
...
**Deprecated for APIKeyExpirationEpoch parameter's. In the future CreateAPIKey parameter replaces this behavior.**
